...
local function mixed_indent_file()
  ...
  if not tab_indent_info then
    return false
  end

  local space_indent_info = count_search [[\v^ +]]
  if not space_indent_info then
    return false
  end
...
local function format_count_search(count_search_info, label)
  if count_search_info.total < 1 then
    return ""
  end
...
local function format_trailing(info)
  return format_count_search(
    info,
    vim.fn.winwidth(0) > ws_shorten_width and "trailing" or "TR"
  )
end
...
local function format_mixed_indent_line(info)
  return format_count_search(
    info,
    vim.fn.winwidth(0) > ws_shorten_width and "mixed-indent-line" or "ML"
  )
end
...
local function whitespace_checks()
  ...
  if
    not trailing_info
    and not mixed_indent_line_info
    and not mixed_indent_file_info
  then
    return ""
  end
